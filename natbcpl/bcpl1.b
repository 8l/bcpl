...
{ LET treesize = 0
   ...
   AND argform = "FROM/A,TO/K,VER/K,SIZE/K,TREE/S,NONAMES/S,*
                 *D1/S,D2/S,OENDER/S,EQCASES/S,BIN/S,XREF/S,DEFS/S,HDRS/K"
   LET stdout = output()
   ...
   hdrs    := argv!13                     // HDRS, default BCPLHDRS
   UNLESS hdrs DO hdrs := "BCPLHDRS"
sawritef("bcpl: calling findinput(%s)*n", argv!0)
   sourcestream := findinput(argv!0)      // FROM
sawritef("bcpl: calling findinput(%s)=>%n*n", argv!0, sourcestream)
...
//writef("trying: GET *"%s*" in %s*n", charv, hdrs)
  ...
  { LET ext = ".h"
    ...
    filename%0 := n
//sawritef("trying: GET *"%s*" in %s*n", filename, hdrs)
    stream := pathfindinput(filename, hdrs)  // MR 10/6/04
  }
