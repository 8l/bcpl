      ...
      { DEFAULT: GOTO err
         ...
         CASE 1:  // ordinary item
             ...
             ELSE TEST rdch()='*n' & compstring("?", w)=0
                  THEN { writef("%s: ", keys) // help facility
                          ENDCASE
                       }
                  ELSE unrdch()
...
{ LET p, q = ?, rootnode!rtn_blklist
   ...
   { p := q
      WHILE (!p & freebit) = 0 DO  // chain through used blocks
       TEST !p=0 THEN RESULTIS 0   // end of block list
                 ELSE p := p + !p
...
AND freevec(p) BE p!-1 := p!-1 | freebit
...
AND unloadseg(segl) BE UNTIL segl=0 DO { LET s = !segl
                                          freevec(segl)
                                          segl := s
                                       }
**************************************************************/
...
AND mkobj(upb, fns) = VALOF // object making function (should be in BLIB)
