// (c)  Copyright:  Martin Richards  29 May 2004
...
{ // Return to the CLI with a return code.
  // It must be called from the command's coroutine,
    ...
    { DEFAULT:    wrch(type)
      ...
      CASE 'D':   IF m DO
                  { // Write a number of the form nnn.nn
                    ...
                    writed(arg/scale, n-1-m)
                    wrch('.')
                    writez( ABS arg REM scale, m)
...
{ randseed := randseed*2147001325 + 715136305
  RESULTIS (ABS(randseed/3)) REM upb + 1
}
...
//sawritef("BLIB: dat_to_string: entered*n")
...
{ LET tv    = VEC 5
  ...
  MANIFEST { secsperday = 60*60*24 }

  sys(Sys_ftime, tv)

  secs  := tv!1 - 2922*secsperday  // Change from Unix to Tripos epoc
  msecs := tv!2
  // secs = (unsigned) seconds since 00:00:00 on 1 Jan 1978
  // **** It will oveflow on 06:28:15 Friday 6 Feb 2114 ****
  // secs := #xFFFFFFFF  // Corresponds to 06:28:15 Friday 6 Feb 2114

  // secsperday*10000 = 864_000_000
  // The following loop will never do more than 3 iterations
  WHILE secs<0 DO { // To avoid overflow problems
    secs, days := secs-secsperday*10000, days+10000
    //sawritef("days=%n*n", days)
  }
  days  := days + secs / secsperday
  secs  := secs REM secsperday
  mins  := secs / 60
  secs  := secs REM 60
