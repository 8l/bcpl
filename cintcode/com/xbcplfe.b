// This is an extend version of the BCPL syntax analyser

// Implemented by Martin Richards (c) 17 May 2013


/* Change history

17/05/13
Added BITSPERBCPLWORD
...
14/07/10
Started major changes to add the floating point and assignment
operators and the <> operator. Added the FLTOP, SELLD abd SELST
OCODE statements.
    ...
    { DEFAULT:
      ...
      CASE 'Z':
              ...
              { DEFAULT: RETURN
                ...
                // Some operators words that can become assignment operators

                CASE s_rem:    assop := s_assrem;    GOTO checkass
                ...
                CASE s_neqv:   assop := s_assneqv;   GOTO checkass
              }
...
{ LET a, op = 0, token
   ...
   { DEFAULT: synerr("Error in expression")
      ...
      CASE s_fsub:   a := rnexp(5)
                     TEST h1!a=s_fnum THEN h2!a := - h2!a
                                      ELSE a := mk2(s_fneg, a)
                     RESULTIS a
...
{ LET size, ln = 0, 0
  ...
  { CASE s_number:
    ...
    CASE s_fnum:   writef("FNUM: %ne%n", h2!x, h3!x); RETURN
    ...
    CASE s_assmul:CASE s_assdiv:CASE s_assrem:
    CASE s_assadd:CASE s_asssub:
    CASE s_asslshift:CASE s_assrshift:CASE s_asslogand:CASE s_asslogor:
    CASE s_asseqv:CASE s_assneqv:
    ...
    CASE s_table:CASE s_abs:CASE s_fabs:CASE s_fneg:
...
{ DEFAULT:            writef("*nopname = %n*n", op)
  ...
  CASE s_assneqv:     RESULTIS "ASSNEQV"
  CASE s_assadd:      RESULTIS "ASSADD"
  CASE s_assrem:      RESULTIS "ASSREM"
...
{ LET op, sfop, sw = ?, ?, FALSE
  ...
  { DEFAULT: trnerr("Compiler error in Trans, op = %s", opname(op)); RETURN
    ...
    CASE s_assvecap:  op, sfop := s_vecap, sf_vecap;   GOTO doassign
    CASE s_assfmul:   op, sfop := s_fmul , sf_fmul ;   GOTO doassign
    CASE s_assfdiv:   op, sfop := s_fdiv, sf_fdiv;     GOTO doassign
    CASE s_assfadd:   op, sfop := s_fadd, sf_fadd;     GOTO doassign
    CASE s_assfsub:   op, sfop := s_fsub, sf_fsub;     GOTO doassign
    CASE s_assmul:    op, sfop := s_mul, sf_mul;       GOTO doassign
    CASE s_assdiv:    op, sfop := s_div, sf_div;       GOTO doassign
    CASE s_assrem:    op, sfop := s_rem, sf_rem;       GOTO doassign
    CASE s_assadd:    op, sfop := s_add, sf_add;       GOTO doassign
    CASE s_asssub:    op, sfop := s_sub, sf_sub;       GOTO doassign
    CASE s_asslshift: op, sfop := s_lshift, sf_lshift; GOTO doassign
    CASE s_assrshift: op, sfop := s_rshift, sf_rshift; GOTO doassign
    CASE s_asslogand: op, sfop := s_logand, sf_logand; GOTO doassign
    CASE s_asslogor:  op, sfop := s_logor, sf_logor;   GOTO doassign
    CASE s_asseqv:    op, sfop := s_eqv, sf_eqv;       GOTO doassign
    CASE s_assneqv:   op, sfop := s_neqv, sf_neqv;     GOTO doassign

    CASE s_ass:        op, sfop := 0, 0;               GOTO doassign

doassign:
      context, comline := x, h4!x
      assign(h2!x, h3!x, op, sfop)
      trnext(next)
      RETURN
...
{ LET op = h1!x
  ...
  { DEFAULT:
    ...
    //   h2!x  of  h3!x
    // selector   pointer
    { LET slct = evalconst(h2!x) // Inserted 11/7/01
      LET len = slct>>24         // Field length
      LET sh  = slct>>16 & 255   // Bits to the right of field
      LET offset = slct & #xFFFF // Word offset
      load(h3!x)                 // Compile the pointer
      IF offset DO
      { out2(s_ln, offset)       // Add offset if necessary
        out1(s_add)
      }
      TEST len=0 & sh=0
      THEN out1(s_rv)
...
AND assign(x, y, op, sfop) BE
// Compile a simultaneous assigment
// op is 0 or an OCODE expression operator allowed in assignments, ie
// s_mul, s_div, etc
// sfop is 0 or one of the SELST operators allowed in assignments, ie
// sf_mul, sf_div, etc
{ IF x=0 | y=0 DO { trnerr("Compiler error in assign")
  ...
  { CASE s_comma:
      ...
      assign(h2!x, h2!y, op, sfop) // Do the assignments from left to right
      assign(h3!x, h3!y, op, sfop)
      RETURN
    ...
    CASE s_of:
    //   h2!x   of   h3!x   op:=   y
    // selector    pointer  op    RHS
    // op is: mul, div, rem, add, sub,
    //        lshift, rshift, logand, logor, eqv, neqv
    // op can be vecap, fmul, fdiv, fadd or fsub if
    // the destinantion is a full word, if if mask=-1
    { LET slct = evalconst(h2!x) // Inserted 11/7/01
      LET len = slct>>24         // Field size
      LET sh  = slct>>16 & 255   // Shift
      LET offset = slct & #xFFFF // Offset
//writef("Compiling (SLCT %n:%n:%n) OF x %s:= y*n",
//          len, sh, offset, sfname(op))

      load(y)
      load(h3!x)           // Get the pointer
      IF offset DO
      { out2(s_ln, offset) // Add offset if necessary
        out1(s_add)
      }
      IF op=0 & len=0 & sh=0 DO  // No op and full word field
      { out1(s_stind)
        ssp := ssp-2
        RETURN
      }
      // Non null op or not a full word field
      IF len | sh DO
        // It is not a full word field
        IF op=sf_fmul | op=sf_fdiv | op=sf_fadd | op=sf_fsub DO
          trnerr("Floating point %s assignment to a non full word field",
                   sfname(op))
      out4(s_selst, sfop, len, sh) 
      ssp := ssp - 2
      RETURN
    }
    ...
    CASE s_byteap:
      ...
      THEN { // It is not a full word field
             IF op=sf_fmul | op=sf_fdiv | op=sf_fadd | op=sf_fsub DO
               trnerr("Floating point %s assignment to a non full word field",
                       sfname(op))
             load(x) // Not good since h2!x and h3!x gets evaluated twice
             load(y)
             out1(op)
             ssp := ssp-1
           }
...
{ LET op, str = h1!x, ""
  ...
  { DEFAULT:  prctxte(x, 4, 0); RETURN
    ...
    CASE s_assrem:    str := "MOD"; GOTO case_ass
    ...
    CASE s_assneqv:   str := "XOR"; GOTO case_ass
...
{ LET op, str = h1!x, ""
  ...
  { DEFAULT: ENDCASE
    ...
    CASE s_fnum:   writef("#%ne%n", h2!x, h3!x); RETURN
  ...
  { DEFAULT: ENDCASE
    ...
    CASE s_of:     str := "::";  GOTO case_of
    CASE s_byteap: str := "%";   GOTO case_of
    CASE s_vecap:  str := "!";   GOTO case_of
case_of:
         prctxte(h2!x, d-1, 10)
         writes(str)
         prctxte(h3!x, d-1, 10)
    ...
    CASE s_rv: str := "!";   GOTO case_rv
    CASE s_lv: str := "@";   GOTO case_rv
case_rv:
         writes(str)
         prctxte(h2!x, d-1, 10)
         RETURN
  ...
  SWITCHON op INTO
  { DEFAULT: ENDCASE
    CASE s_fmul:  str := "#**"; GOTO case_mul 
    CASE s_fdiv:  str := "#/";  GOTO case_mul 
    CASE s_mul:   str := "**";  GOTO case_mul 
    CASE s_div:   str := "/";   GOTO case_mul 
    CASE s_rem:   str := "MOD"; GOTO case_mul 
case_mul :
         prctxte(h2!x, d-1, 9)
         writes(str)
         prctxte(h3!x, d-1, 9)
  ...
  SWITCHON op INTO
  { DEFAULT: ENDCASE
    CASE s_fadd:   str := "#+"; GOTO case_add
    CASE s_fsub:   str := "#-"; GOTO case_add
    CASE s_add:    str := "+";  GOTO case_add
    CASE s_sub:    str := "-";  GOTO case_add
case_add:
         prctxte(h2!x, d-1, 8)
         writes(str)
         prctxte(h3!x, d-1, 8)
    ...
    CASE s_fneg:   str := "#-";   GOTO case_neg
    CASE s_neg:    str := "-";    GOTO case_neg
    CASE s_fabs:   str := "FABS"; GOTO case_neg
    CASE s_abs:    str := "ABS";  GOTO case_neg
case_neg:
         writes(str)
         prctxte(h2!x, d-1, 8)
         RETURN
  ...
  SWITCHON op INTO
  { DEFAULT: ENDCASE
    CASE s_feq:   str := "#=";  GOTO case_eq
    CASE s_fne:   str := "#~="; GOTO case_eq
    CASE s_fls:   str := "#<";  GOTO case_eq
    CASE s_fgr:   str := "#>";  GOTO case_eq
    CASE s_fle:   str := "#<="; GOTO case_eq
    CASE s_fge:   str := "#>="; GOTO case_eq
    CASE s_eq:    str := "=";   GOTO case_eq
    CASE s_ne:    str := "~=";  GOTO case_eq
    CASE s_ls:    str := "<";   GOTO case_eq
    CASE s_gr:    str := ">";   GOTO case_eq
    CASE s_le:    str := "<=";  GOTO case_eq
    CASE s_ge:    str := ">=";  GOTO case_eq
case_eq:
         prctxte(h2!x, d-1, 7)
         writes(str)
         prctxte(h3!x, d-1, 7)
  ...
  SWITCHON op INTO
  { DEFAULT: ENDCASE
    CASE s_lshift:   str := "<<";  GOTO case_lshift
    CASE s_rshift:   str := ">>";  GOTO case_lshift
case_lshift:
         prctxte(h2!x, d-1, 6)
         writes(str)
         prctxte(h3!x, d-1, 6)
  ...
  SWITCHON op INTO
  { DEFAULT: ENDCASE
    CASE s_eqv:  str := "EQV"; GOTO case_eqv
    CASE s_neqv: str := "XOR"; GOTO case_eqv
case_eqv:
         prctxte(h2!x, d-1, 2)
         writes(str)
         prctxte(h3!x, d-1, 2)
