...
**
** (c) Copyright:  Martin Richards  25 Jan 2012
**
*/
...
{ int rc;
  ...
  { int membytes = (memupb+tallyupb+vecstatsvupb+7)<<B2Wsh;
    ...
    if( tracing ) printf("W=%d %8x\n", (BCPLWORD)W, (BCPLWORD)W);
#else
  ...
  { BCPLWORD seg = loadseg("syscin/boot");
    if(seg==0) {
      ...
      PRINTF("environment variables such as BCPLROOT and BCPLPATH\n");
...
{ BCPLWORD list  = 0;
  ...
  //  /* look in <BCPLROOT>/cin, ie preprend cin/ to file */
  //  /*PRINTFS("loadseg: searching for %s in $BCPLROOT/cin\n", file); */
  //  fp = pathinput(catstr2c_str("cin/", file, chbuf), "BCPLROOT");
  //}
  if(fp==NULL) return 0;
...
BCPLWORD doflt(BCPLWORD op, BCPLWORD a, BCPLWORD b, BCPLWORD c) {
  ...
  switch (op) {
    ...
    case fl_mul:
    { float res = N2F a * N2F b;
      //x.i = a; y.i = b;
      //printf("fl_mul: %5.1f * %5.1f => ", x.f, y.f);
      //x.f = x.f * y.f; 
      //printf("         => %5.1f\n", x.f);
      return F2N res;
      //return x.i;
