...
/* cintsys64.h contains machine/system dependent #defines  */
#include "cintsys64.h"
...
#ifdef MEMCHKyes
#define MC1(a) if((unsigned BCPLWORD)a>memupb){W[3]=a; res=12; pc--;  goto ret; }
#define MC2(a) if((unsigned BCPLWORD)a>memupb){W[3]=a; res=12; pc-=2; goto ret; }
#define MC3(a) if((unsigned BCPLWORD)a>memupb){W[3]=a; res=12; pc-=3; goto ret; }
#define MC5(a) if((unsigned BCPLWORD)a>memupb){W[3]=a; res=12; pc-=5; goto ret; }
#else
#define MC1(n)
#define MC2(n)
#define MC3(n)
...
#define GW(x) (BCPLINT32)((((((WD B[x]<<8)|B[x+1])<<8)|B[x+2])<<8)|B[x+3])
#else
#define GH(x) ((WD B[x+1]<<8) | B[x])
#define GW(x) (BCPLINT32)((((((WD B[x+3]<<8)|B[x+2])<<8)|B[x+1])<<8)|B[x])
#endif
...
{ //printf("cintasm: entered regs=%lld\n", regs);
#else
int interpret(BCPLWORD regs, BCPLWORDpt mem)
{ //printf("interpret: entered regs=%lld\n", regs);
#endif
{  BCPLWORDpt W = mem;
   ...
   //printf("cintasm or interpret: entered\n");
   /*   tracing = 1; */
...
if(--icount<=0) {
  ...
  icount = 10000;
  // icount     bench100 time under 64-bit Cintcode using gcc -O4
  //   1      955.090
  //   10     106.200
  //   100     18.760
  //   1000     9.980
  //   10000    9.050
  //   100000   9.020
  //            8.790 with this code omitted (gcc -O1)
...
{  default:
   ...
   case F_selst: // SLCT len:sh:0 OF <arg1> op:= <arg2>
     ...
     { BCPLWORD *ptr = &W[a];
       ...
       case sf_fmult:    x.i = val; y.i = b;
                         x.f = x.f * y.f;
                         val = x.i;               break;
       case sf_fdiv:     x.i = val; y.i = b;
                         x.f = x.f / y.f;
                         val = x.i;               break;
       case sf_fplus:    x.i = val; y.i = b;
                         x.f = x.f + y.f;
                         val = x.i;               break;
       case sf_fminus:   x.i = val; y.i = b;
                         x.f = x.f - y.f;
                         val = x.i;               break;
       case sf_mult:     val *= b;                break;
       case sf_div:      val /= b;                break;
       case sf_rem:      val %= b;                break;
       case sf_plus:     val += b;                break;
       case sf_minus:    val -= b;                break;
