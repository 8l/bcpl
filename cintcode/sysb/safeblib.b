...
{ MANIFEST
  ...
  LET w        = argv   // w is a moving pointer into argv
  LET numbargs = ?
...
// Check for prompted arguments.

               FOR i = 0 TO numbargs-1 IF 0<=argv!i<=control DO
               { // Unset argument found
                 LET a = argv!i

                 // If an error has been detected give up
                 UNLESS status BREAK

                 IF ((a & prompt) ~= 0) DO
                 { // The argument has the /P qualifier
                   IF cis!scb_type = scbt_console &
                      cos!scb_type = scbt_console DO
                   { // Both current input and output are
                     ...
                     { CASE 0: // endstreamch
                       ...
                       CASE 2: // Quoted item
                               ...
                               UNLESS (a & (switch|number)) ~= 0 DO
                               { // Neither /S or /N present
                                 argv!i := w
                                 w := w + w%0/bytesperword + 1
                                 wsize := size - (w - argv)     
                               }
                               itemtype := rditem(w, wsize)
                               REPEATUNTIL itemtype=0 | // endstreamch
                                           itemtype=3 | // newline
                                           itemtype=4   // semicolon
                               LOOP

                       CASE 3: // newline
                               // Do not set this argument.
                               argv!i := 0
                               // But if it was required indicate an error
                               IF (a & required) ~= 0 DO status := FALSE
                               LOOP
               ...
               result2 := 0
               // Setting result2 to the number of arguments given
               // is not worth doing. So this feature is removed.  MR 1/1/14
