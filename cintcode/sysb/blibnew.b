...
AND rdargs(key, argv, size) = VALOF
{ LET res = rdargs1(key, argv, size)
  LET r2 = result2
  //sawritef("rdargs(*"%s*", %n, %n) => %n result2=%n*n", key, argv, size, res, r2)
  result2 := r2
  RESULTIS res
}

AND rdargs1(keys, argv, size) = VALOF
{ MANIFEST
  { a_bit =  1            // /A
    k_bit =  2            // /K
    s_bit =  4            // /S
...
//FOR i = 0 TO argmax DO sawritef("arg%n type: %b5*n", i, argtype!i)
//abort(1000)
...
//IF itemtype=1 DO
//  sawritef("itemtype=%n item=%s wsize=%n argmax=%n*n", itemtype, w, wsize, argmax)
//IF itemtype=2 DO
//  sawritef("itemtype=%n item=*"%s*" wsize=%n argmax=%n*n", itemtype, w, wsize, argmax)
//abort(1000)
...
skip:
               ...
               result2 := 0
//sawritef("returning from rdargs with result w=%n*n", w)
               RESULTIS w // Point to first unused word in argv

      CASE 1:  // Unquoted item

               argno := findarg(keys, w)
//sawritef("findarg(*"%s*", *"%s*") => %n*n", keys, w, argno)
//abort(1000)
               TEST argno >= 0
...
//sawritef("%s is a positional argument, argno=%n*n", w, argno)
//abort(1000)
              ...
              { UNLESS string_to_number(w) GOTO err
                ...
                argtype!argno := argtype!argno | d_bit
                w  := w + 1
//sawritef("arg%n is numeric and has been given value %n*n", argno, argv!argno)
//abort(1000)
                LOOP
              }
              ...
              argtype!argno := argtype!argno | d_bit
              w := w + w%0/bytesperword + 1
//sawritef("arg%n and has been given value %s*n", argno, argv!argno)
//abort(1000)
              LOOP
