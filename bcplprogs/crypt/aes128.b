...
{ LET rcon = 1
  ...
  FOR i = 1 TO 10 DO
  { LET p = @Rkey!(4*i) // Pointer to space for round key i
    LET q = p-4         // Pointer to roundkey i-1

    p%0 := q%0 XOR sbox%(q%13) XOR rcon
    p%1 := q%1 XOR sbox%(q%14)
    p%2 := q%2 XOR sbox%(q%15)
    p%3 := q%3 XOR sbox%(q%12)
...
{ LET argv = VEC 50
  ...
  LET in  = VEC 63
  LET out = VEC 63
  LET v   = VEC 4*10+3 // For the key schedule of 11 keys
