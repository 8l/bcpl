...
AND insert1(x) BE
{ MANIFEST { Bits=4; N=1<<Bits; Mask=N-1 }
  x := mtf(x, mtfbuf, mtfbufupb)
  UNTIL 0<=x<=Mask DO
  { insertbyte((x&Mask)+N)
    x := x>>Bits
  }
...
{ STATIC { prev=0; count=0 }
  ...
  t!prev := x
  prev := (x + (prev>>5)) & 127
  //IF x=y DO { count := count+1; RETURN }
  UNTIL count=0 DO
  { count := count-1
    insertval(254+(count&1))
    count := count>>1
  }
  insertval(x)
}
