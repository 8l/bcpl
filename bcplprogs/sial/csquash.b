...
// Written by Martin Richards  23 July 1998
...
{ LET argv = VEC 50
  ...
  { writes("Bad arguments for csquash*n")
    RESULTIS 10
  }

  infilename := argv!0 -> argv!0, "Cstr"
  ...
  { bitcount := huffgen(frqv, lenv, 256)
    ...
    FOR i =  0 TO 15 DO codelen!i := 2 + i/2 // For constant Huffman table
...
AND insert(x) BE
{ STATIC { hash=0; uppercase=FALSE }
  LET predx = mtfbuf!hash
  IF 'a'<=x<='z' &  uppercase DO
  { insertval(255)
    uppercase := FALSE
  }
  IF 'A'<=x<='Z' DO
...
{ frqv!x := frqv!x + 1
  data!datap := x
  datap := datap+1
  IF (debug&64)>0 DO writef("%i3*n", x)
}
