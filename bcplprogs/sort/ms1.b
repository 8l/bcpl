...
LET merge(p, q, r) BE UNTIL p>=q | q>=r DO
{ // a ----> b ----> 
  // p       q       r
  ...
  // First advance p until it equals q or points to
  // an element > !q
  WHILE p!0 <= x DO p := p+1
  IF p=q RETURN
  IF !p=b DO { p := p+1; LOOP }

  // Now  p<q and q<r and !p > !q

  // Next advance s while s<r and !s < !p
  a := !p
  WHILE s<r & !s < a DO s := s+1   // ? <=
  ...
  // Swap some elements starting at p with those starting at q
  // but don't let p advance beyond q

  t := q
  { LET x, y = !p, !t
    IF y >= a BREAK
    !p, !t := y, x     // Swap elements
    p, s := p+1, s+1
  } REPEATUNTIL p>=q | s>=r
