...
{ LET t = TABLE //         Cintcode instruction executed
  ...
  //  test data  result  lsbn  lsbna lsbnb lsbnc lsbnd lsbne lsbnf lsbng lsbnh lsbni lsbnj

     #x00000000, // -1:    3   39    3   10   30   19   19   19   32   32   32 
...
{ LET n = lsbn(s)
  writef("%x8 %i2: ", s, n)
  try(s, lsbn);  try(s, lsbna); try(s, lsbnb); try(s, lsbnc)
  try(s, lsbnd); try(s, lsbne); try(s, lsbnf); try(s, lsbng)
  try(s, lsbnh); try(s, lsbni); try(s, lsbnj)
  newline()
}

AND try(w, f) BE writef(" %i3%c", instrcount(f, w), lsbn(w)=f(w) -> ' ', '#')

AND lsbn(s) = s=0 -> 0, VALOF
{ LET n = 0
  { UNLESS (n&s)=0 RESULTIS n
    bit, n := bit<<1, n+1
  } REPEATWHILE n
  RESULTIS -1
}
...
{ LET t = TABLE
  ...
  IF (w&#xFF000000) RESULTIS t!(w>>24) + 24
  IF (w&#x00FF0000) RESULTIS t!(w>>16) + 16
  IF (w&#x0000FF00) RESULTIS t!(w>> 8) +  8
  RESULTIS t!w
}
  
AND lsbnb(w) = w=0 -> 0, VALOF
{ LET r = w = (w & -w) -> 0, 1
  LET a = ?
...
AND lsbng(w) = VALOF  // The fastest method
{ LET t = TABLE
