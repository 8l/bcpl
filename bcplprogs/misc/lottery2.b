...
AND selectl(n, wh, wl, choice, ) BE
$( LET bit = w & -w
   IF n=0 | w=0 DO selecth(n, #x1FFFF, 0, bits(choice&bl))
   select(n-1, w-bit, choice+bit)
   select(n,   w-bit, choice)
$)

AND selecth(n, wh, choice, count) BE
$( LET bit = w & -w
   IF n=0 | w=0 DO selecth(n, #x1FFFF, 0, bits(choice&bl))
   select(n-1, w-bit, choice+bit)
   select(n,   w-bit, choice)
